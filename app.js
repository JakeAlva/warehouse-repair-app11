...
/* Rack Repair Survey - no backend, everything in localStorage */
...
function renderFrameList(){
  ...
  if(!survey.frames.length){
    list.innerHTML = '<div class="muted">No frame types added yet.</div>'; return;
  }
  survey.frames.forEach((f, idx)=>{
    ...
    edit.addEventListener('click', ()=>{
      // Load values back into form for quick edit
      ...
      // Remove from list so saving adds as new/updated at end
...
function populateFrameDropdown(){
  ...
  if(!survey.frames.length){
    const o = document.createElement('option'); o.text='No frames yet'; dd.appendChild(o); return;
  }
  survey.frames.forEach((f,i)=>{
    const o = document.createElement('option');
    o.value = i; o.text = f.name || `Frame #${i+1}`;
    dd.appendChild(o);
  });
}
...
$('#addFrameBtn').addEventListener('click', ()=>{
  ...
  if(!f.depth || !f.postWidth || !f.backer || !f.style || !f.punch){
...
$('#imgModal').addEventListener('click', (e)=>{ if(e.target.id==='imgModal') $('#imgModal').classList.add('hidden'); });

/* -------- LOCATIONS TAB -------- */
// Build the height options from 24" to 192" inclusive
...
function renderLocList(){
  ...
  if(!survey.locations.length){
    list.innerHTML = '<div class="muted">No damage locations logged yet.</div>'; return;
  }
  survey.locations.forEach((loc, idx)=>{
    ...
    del.addEventListener('click', ()=>{
      if(confirm('Delete this entry?')){ survey.locations.splice(idx,1); saveSurvey(); renderLocList(); }
    });
...
$('#addLocationBtn').addEventListener('click', ()=>{
  ...
  const entry = {
    ...
    beams: {...survey.beams} // snapshot beams at time of entry
  };
...
function exportToExcelHTML(){
  // Build HTML table string that Excel can open as .xls
...
$('#newSurveyBtn').addEventListener('click', ()=>{
  if(confirm('Start a new survey? This will clear all current survey data.')){
    ...
    // Reset UI
    startFields.forEach(id=> $('#'+id).value = '');
...
/* ---- Load previously saved fields into UI ---- */
(function hydrate(){
  // Start tab fields already hydrated above
  renderFrameList(); populateFrameDropdown(); renderLocList();
})();
